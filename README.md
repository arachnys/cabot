...
4.  Run `fab provision -H root@your.server.hostname` from your local clone. This will install dependencies on the new server and create a new `ubuntu` user that will be able to connect over SSH (we do this for compatibility with Amazon's Ubuntu AMIs).
